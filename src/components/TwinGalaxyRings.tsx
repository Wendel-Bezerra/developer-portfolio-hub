...
// Twin Galaxy Rings — Originkit (adaptado: sem framer-motion; hover/press são
// suavizados com decaimento exponencial direto no loop de render).
...
// Constante de tempo do smoothing de hover/press (substitui o tween de 0.8s).
const EASE_RATE = 5;
...
export default function TwinGalaxyRings(props: Props) {
    ...
    useEffect(() => {
        ...
        // Eventos na window: o host fica atrás do conteúdo com pointer-events
        // desligado, então o hover é rastreado globalmente e o alvo é a área
        // do host na tela.
        const onMove = (e: PointerEvent) => {
            ...
            if (!inside) pointer.current.pressTarget = 0;
        };
        const onDown = () => {
            if (pointer.current.target === 1) pointer.current.pressTarget = 1;
        };
        const onUp = () => {
        ...
        const frame = (now: number) => {
            ...
            p.active += (p.target - p.active) * k;
            p.press += (p.pressTarget - p.press) * k;
